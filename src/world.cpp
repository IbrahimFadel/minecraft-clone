...
void World::generateChunk(glm::ivec2 position)
{
    ...
    // chunk->setBlock(glm::ivec3(0, 0, 0), BlockType::Stone);
...
void World::renderBlockMeshes(ivec2 cameraPos, const std::unique_ptr<Shader> &shader)
{
    for (auto const &[pos, chunk] : chunks)
    {
        if (chunkInRenderDistance(cameraPos, pos))
            chunk->renderBlockMesh(shader);
    }
}

void World::renderWaterMeshes(ivec2 cameraPos, const std::unique_ptr<Shader> &shader)
{
    for (auto const &[pos, chunk] : chunks)
    {
        if (chunkInRenderDistance(cameraPos, pos))
            chunk->renderWaterMesh(shader);
    }
}

void World::generateChunksAroundPosition(glm::ivec2 position)
{
    if (chunks[position] == nullptr)
        generateChunk(position);

    std::vector<ivec2> positionsToGenerate;
    for (int x = position.x - renderDistance; x < position.x + renderDistance; x++)
    {
        for (int y = position.y - renderDistance; y < position.y + renderDistance; y++)
        {
            ivec2 pos = {x, y};
...
bool World::chunkInRenderDistance(ivec2 cameraPos, ivec2 chunkPos)
{
    return (chunkPos.x < cameraPos.x + renderDistance) && (chunkPos.x > cameraPos.x - renderDistance) && (chunkPos.y < cameraPos.y + renderDistance) && (chunkPos.y > cameraPos.x - renderDistance);
}